...
$title.Text = 'ODC Studio'; $title.Left = 28; $title.Top = 24; $title.AutoSize = $true; $title.Font = New-Object Drawing.Font('Segoe UI',16,[Drawing.FontStyle]::Bold); $title.ForeColor = $text
...
$chip.Text = 'ODC1'; $chip.Width = 62; $chip.Height = 28; $chip.Top = 24; $chip.Left = 1000; $chip.Anchor = 'Top,Right'; $chip.TextAlign='MiddleCenter'; $chip.BackColor=[Drawing.Color]::FromArgb(239,246,255); $chip.ForeColor=[Drawing.Color]::FromArgb(29,78,216); $chip.Font=New-Object Drawing.Font('Segoe UI',8,[Drawing.FontStyle]::Bold)
...
$verLabel.Text = 'ODC Studio PowerShell · SDK 1.1.0'
[void]$status.Items.Add($verLabel)
$form.Controls.Add($status)

$tabs = New-Object Windows.Forms.TabControl
$tabs.Dock = 'Fill'; $tabs.Padding = New-Object Drawing.Point(18,8); $tabs.Font = New-Object Drawing.Font('Segoe UI',9,[Drawing.FontStyle]::Bold)
$form.Controls.Add($tabs)
$tabs.BringToFront(); $status.BringToFront(); $top.BringToFront()

function Add-Tab([string]$name){$t=New-Object Windows.Forms.TabPage;$t.Text=$name;$t.BackColor=$canvas;$t.Padding=New-Object Windows.Forms.Padding(18);[void]$tabs.TabPages.Add($t);return $t}
...
function Card($p,[int]$x,[int]$y,[int]$w,[int]$h,[string]$title,[string]$subtitle=''){$g=New-Object Windows.Forms.GroupBox;$g.Text=$title;$g.Left=$x;$g.Top=$y;$g.Width=$w;$g.Height=$h;$g.Anchor='Top,Left,Right';$g.BackColor=[Drawing.Color]::White;$g.ForeColor=$text;$g.Font=New-Object Drawing.Font('Segoe UI',9,[Drawing.FontStyle]::Bold);$p.Controls.Add($g);if($subtitle){$l=New-Object Windows.Forms.Label;$l.Text=$subtitle;$l.Left=16;$l.Top=28;$l.Width=$w-32;$l.Height=20;$l.ForeColor=$muted;$l.Font=New-Object Drawing.Font('Segoe UI',8);$g.Controls.Add($l)};return $g}
function Lbl($p,$text,$x,$y){$l=New-Object Windows.Forms.Label;$l.Text=$text;$l.Left=$x;$l.Top=$y;$l.AutoSize=$true;$l.ForeColor=$muted;$l.Font=New-Object Drawing.Font('Segoe UI',8,[Drawing.FontStyle]::Bold);$p.Controls.Add($l);return $l}
...
function Multi($p,$x,$y,$w,$h){$t=New-Object Windows.Forms.TextBox;$t.Left=$x;$t.Top=$y;$t.Width=$w;$t.Height=$h;$t.Multiline=$true;$t.ScrollBars='Both';$t.AcceptsTab=$true;$t.Font=New-Object Drawing.Font('Consolas',9);$t.BorderStyle='FixedSingle';$t.BackColor=[Drawing.Color]::FromArgb(251,252,254);$p.Controls.Add($t);return $t}
function Btn($p,$text,$x,$y,$handler,[bool]$primary=$false){$b=New-Object Windows.Forms.Button;$b.Text=$text;$b.Left=$x;$b.Top=$y;$b.Height=34;$b.AutoSize=$true;$b.FlatStyle='Flat';$b.Font=New-Object Drawing.Font('Segoe UI',8,[Drawing.FontStyle]::Bold);if($primary){$b.BackColor=$blue;$b.ForeColor=[Drawing.Color]::White;$b.FlatAppearance.BorderColor=$blue}else{$b.BackColor=[Drawing.Color]::White;$b.ForeColor=$text;$b.FlatAppearance.BorderColor=[Drawing.Color]::FromArgb(203,213,225)};$b.Add_Click($handler);$p.Controls.Add($b);return $b}
...
function Fail($e){$statusLabel.Text='Falha na operação';[Windows.Forms.MessageBox]::Show($e.Exception.Message,'ODC Studio',[Windows.Forms.MessageBoxButtons]::OK,[Windows.Forms.MessageBoxIcon]::Error)|Out-Null}

# Criar
$t=Add-Tab 'Criar container'
$c=Card $t 18 18 1010 560 'Novo container ODC' 'Selecione a origem, destino e metadata opcional.'
Lbl $c 'ARQUIVO DE ORIGEM' 18 62|Out-Null;$src=Txt $c 18 84 820;Btn $c 'Selecionar' 850 81 {$p=Pick-Open;if($p){$src.Text=$p}}|Out-Null
...
Btn $c 'Criar ODC' 18 500 {try{$m=@{};if($meta.Text.Trim()){$o=$meta.Text|ConvertFrom-Json;$o.psobject.Properties|ForEach-Object{$m[$_.Name]=$_.Value}};New-OdcFile $src.Text $dst.Text $m $compress.Checked;$statusLabel.Text="Container criado: $($dst.Text)";Msg 'Container ODC criado com sucesso.'}catch{Fail $_}} $true|Out-Null
